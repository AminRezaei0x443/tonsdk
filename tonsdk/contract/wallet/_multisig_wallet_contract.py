...
class MultiSigOrder:
    ...
    def sign(self, owner_id: int, secret_key: bytes):
        ...
        self.signatures[owner_id] = sign_message(
            bytes(signing_hash), secret_key
        ).signature
        return signing_hash
    ...
    def add_signature(self, owner_id: int, signature: bytes, multisig_wallet):
        signing_hash = self.payload.bytes_hash()
        if not verify_sign(
            public_key=multisig_wallet.options["public_keys"][owner_id],
    ...
    def union_signatures(self, other):
        self.signatures = dict(
            list(self.signatures.items()) + list(other.signatures.items())
        )
...
class MultiSigOrderBuilder:
    def __init__(self, wallet_id, offset=7200, query_id: Optional[int] = None):
        ...
        self.query_id = (
            query_id if query_id is not None else self.generate_query_id(offset)
        )
...
class MultiSigWallet(MultiSigWalletContractBase):
    ...
    def create_transfer_message(
        self, order: MultiSigOrder, private_key: bytes, dummy_signature=False
    ):
    ...
    def create_external_message(
        self, signing_message, private_key, dummy_signature=False
    ):
        signature = (
            bytes(64)
            if dummy_signature
            else sign_message(
                bytes(signing_message.bytes_hash()), private_key
            ).signature
        )
