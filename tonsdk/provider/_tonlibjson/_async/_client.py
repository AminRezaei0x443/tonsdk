...
class AsyncTonlibClient:
    def __init__(self, config, keystore, loop, cdll_path=None, verbosity_level=0):
    ...
    async def init(self, max_restarts=None):
        ...
        keystore_obj = {"@type": "keyStoreTypeDirectory", "directory": self.keystore}
        # create keystore
    ...
    async def set_verbosity_level(self, level):
        request = {"@type": "setLogVerbosityLevel", "new_verbosity_level": level}
        return await self.tonlib_wrapper.execute(request)

    async def raw_get_transactions(
        self, account_address: str, from_transaction_lt: str, from_transaction_hash: str
    ):
        """
        TL Spec:
    ...
    async def raw_create_and_send_query(
        self, destination, body, init_code=b"", init_data=b""
    ):
    ...
    async def raw_estimate_fees(
        self, destination, body, init_code=b"", init_data=b"", ignore_chksig=True
    ):
        query_info = await self._raw_create_query(
            destination, body, init_code, init_data
        )
        ...
        **kwargs,
        ...
        while incomplete:
            ...
            if (result["@type"]) == "error":
                result = await self.raw_get_block_transactions(
